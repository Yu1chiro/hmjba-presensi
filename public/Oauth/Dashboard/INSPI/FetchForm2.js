...
const initializeAppFunctions = (database) => {
    ...
    const removeAllData = async () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const dataRef = ref(database, 'Inspi-Telat');
                try {
    ...
    const extractTambahan = async () => {
        ...
        try {
            ...
            if (snapshot.exists()) {
                ...
                XLSX.utils.book_append_sheet(workbook, worksheet, "Inspi-Terlambat");

                XLSX.writeFile(workbook, "INSPI_TERLAMBAT.xlsx");
                Swal.fire({
    ...
    onChildAdded(dataRef, (snapshot) => {
        ...
        const tableBody = document.getElementById('Inspi-Telat');
        if (tableBody) {
    ...
    function hapusData(nim) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                remove(ref(database, `Inspi-Telat/${nim}`))
                    .then(() => {
                        Swal.fire({
                            icon: 'success',
