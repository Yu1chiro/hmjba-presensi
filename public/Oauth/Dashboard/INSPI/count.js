...
const initializeAppFunctions = (database) => {
    const hitungStatistikKehadiran = () => {
        const presensiInspiRef = ref(database, 'Presensi-Inspi');
        const inspiTelatRef = ref(database, 'Inspi-Telat');
        ...
        onValue(presensiInspiRef, (snapshot) => {
            ...
            if (snapshot.exists()) {
                snapshot.forEach((childSnapshot) => {
                    ...
                    console.log('Presensi-Inspi Keterangan:', keterangan); // Debugging
        ...
        onValue(inspiTelatRef, (snapshot) => {
            ...
            if (snapshot.exists()) {
                snapshot.forEach((childSnapshot) => {
                    ...
                    console.log('Inspi-Telat Status:', status); // Debugging
                    switch (status) {
                        case 'Terlambat':
                            telatCount++;
                            break;
