...
const initializeAppFunctions = (database) => {
    ...
    // Fungsi untuk menghapus data - diperbaiki
    const removeData = async (nim) => {
        const dataRef = ref(database, 'Presensi-Potluck/' + nim);
        ...
        try {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    // Data akan diperbarui otomatis karena onValue listener
                }
            });
        } catch (error) {
            ...
            if (snapshot.exists()) {
                ...
                Object.keys(data).forEach(key => {
                    ...
                    row.innerHTML = `
                        ...
                        <td class="text-center text-sm px-2">${item.kelas}</td>
    ...
    const extractData = async () => {
        ...
        try {
            ...
            if (snapshot.exists()) {
                ...
                XLSX.utils.book_append_sheet(workbook, worksheet, "Presensi-Potluck");
    
                XLSX.writeFile(workbook, "PRESENSI-POTLUCK.xlsx");
                Swal.fire({
    ...
    const migrateData = async () => {
        ...
        const { value: formValues } = await Swal.fire({
            ...
            html:
                ...
                '<option value="Presensi-Potluck">Presensi Potluck</option>' +
                '<option value="Potluck-Telat">Presensi Terlambat</option>' +
                '</select></div>' +
                '</div>',
            focusConfirm: false,
            showCancelButton: true,
        ...
        try {
            ...
            if (snapshot.exists()) {
                ...
                const migrateRef = push(ref(database, 'migrate-potluck'));
